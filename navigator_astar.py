...
Purpose: Implementation of navigator class that uses the A* algorithm to solve
...
class Navigator(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("max_planning_bounds", [80, 80])
        ...
        self.current_path_nodes: list[Node] = []

        # cooldown to avoid too much replanning
        self.last_replan_time = time.time()
        self.replan_cooldown = 0.3 
        ...
        # Timer  to monitor path validity & trigger replannin
        timer_frequency = 20.0
        self.timer_period = 1.0 / timer_frequency
        self.monitor_timer = self.create_timer(
            self.timer_period, self.monitor_callback
        )

        self.get_logger().info("Navigator started with online A* monitoring")
    ...
    def goal_reached_callback(self, goal_reached: Bool):
        ...
        self.is_goal_reached = bool(goal_reached.data)

        if self.is_goal_reached:
            # clear path when goal is reached
            self.current_path_nodes = []
            self.get_logger().info("Goal reached – stopping online replanning")
    ...
    def robot_moving_callback(self, robot_moving: Bool):
        ...
        """
        self.is_robot_moving = bool(robot_moving.data)
    ...
    def planning_callback(self) -> bool:
        """Planning callback
        ...
        if self.robot_position is None or self.robot_orientation is None or self.goal is None:
            self.get_logger().warning(
                "Planning requested but robot pose or goal not available yet"
            )
        ...
        if not self.is_map_loaded or self.map is None:
            self.get_logger().warning("Planning requested but map not available yet")
        ...
        planner.set_start(start)  # set start state
        planner.set_goal(goal)  # set goal state
        ...
        if solved:  # if the query is solved
            ...
            self.current_path_nodes = solution_path
            self.last_replan_time = time.time()
    ...
    def monitor_callback(self):
        ...
        Periodically checks if the current path is still valid with the updated map.
        If the path becomes invalid, it stops the robot and requests a new plan.
        """

        
        if self.goal is None or self.is_goal_cancelled or self.is_goal_reached:
            return

        
        if (
            not self.is_map_loaded
            or self.map is None
        ...
        if not self.current_path_nodes:
            ...
            if now - self.last_replan_time > self.replan_cooldown:
                self.get_logger().info("No current path – trying to replan")
        ...
        lookahead = min(80, len(self.current_path_nodes))

        for i in range(lookahead):
            node = self.current_path_nodes[i]

            if not collision_checker.is_node_free(node):
                self.get_logger().warning(
                    f"Path blocked at step {i} – triggering replanning"
                )
    ...
    def cancel_goal(self):
        ...
        """
        # send message to controller to stop the motion
        self.stop_motion_pub.publish(Bool(data=True))
        self.is_goal_cancelled = True
        self.current_path_nodes = []
        self.get_logger().info("Goal cancelled – robot motion stopped")
